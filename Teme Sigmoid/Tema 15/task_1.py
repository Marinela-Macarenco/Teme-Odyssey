...
class DataContainer:
    ...
    def __add__(self, other):
        if isinstance(other, DataContainer):
            return DataContainer(self.data + other.data)
        else:
...
class DataStatistics:
    ...
    def mean(self):
        total = sum(sum(container.data) for container in self.containers)
        count = sum(len(container) for container in self.containers)
        return total / count
    ...
    def min(self):
        return min(num for container in self.containers for num in container.data)
    
    def sum(self):
        return sum(sum(container.data) for container in self.containers)
# CODUL TĂU VINE MAI SUS:
...
# CODUL TĂU VINE MAI JOS:
class DataFilter:
    def __init__(self, data_containers, data_statistics):
    ...
    def filter_zeros(self):
        return [num for container in self.data_containers for num in container.data if num != 0]
    ...
    def filter_under_mean(self):
        mean_value = self.data_statistics.mean()
        return [num for container in self.data_containers for num in container.data if num > mean_value]
