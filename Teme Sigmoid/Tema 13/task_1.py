...
class Produs:
    ...
    def afisare(self):
        return f"{self.nume} - Pret: {self.pret}, An: {self.an}"
...
class GoogleProduct(AppleProduct):
    def __init__(self, product_type, color):
        super().__init__(product_type, color)

    def combine_products(self, other_product):
        if isinstance(other_product, AppleProduct):
            combined_type = self.product_type + " " + other_product.product_type
            combined_color = other_product.color  # Utilizează culoarea produsului Apple
            return GoogleProduct(combined_type, combined_color)
        elif isinstance(other_product, GoogleProduct):
            combined_type = self.product_type + " " + other_product.product_type
            combined_color = other_product.color  # Utilizează culoarea produsului Google
            return GoogleProduct(combined_type, combined_color)
        else:
            raise TypeError("Can only combine with AppleProduct or GoogleProduct")
# CODUL TĂU VINE MAI SUS:
