...
def task_5(*args, **kwargs):
    ints = sorted([arg for arg in args if isinstance(arg, int)])
    strs = sorted([key for key, value in kwargs.items() if isinstance(value, str)])
    return ints, strs
# CODUL TĂU VINE MAI SUS:
...
def task_13(first, *args):
    ...
    sorted_first = sorted_string(first)
    return all(sorted_string(arg) == sorted_first for arg in args)
# CODUL TĂU VINE MAI SUS:
...
def task_14(sub_string, *args):
    result = [arg for arg in args if sub_string in arg]
    return result
...
def task_16(*args, operation):
    if operation == 'add':
        ...
        raise ValueError("Operatie necunoscuta")
# CODUL TĂU VINE MAI SUS:
...
def task_17(number, **kwargs):
    ...
    def apply_operation(x, op, nums):
        if op == 'add':
            return x + sum(nums)
        elif op == 'sub':
            return x - sum(nums)
        elif op == 'mul':
            return reduce(lambda a, b: a * b, nums, x)
        elif op == 'div':
            try:
                return reduce(lambda a, b: a / b if b != 0 else a, nums, x)
            except ZeroDivisionError:
                return "Error: Division by zero is not allowed"
    
    result = number
    for op, nums in kwargs.items():
        result = apply_operation(result, op, nums)
    return result
# CODUL TĂU VINE MAI SUS:
